...
Full-text search uses Algolia. Geo-distance filtering uses a Haversine formula executed directly in PostgreSQL, avoiding a round-trip to an external service for every search. In the test environment, Algolia is stubbed and falls back to an `ILIKE` query.
